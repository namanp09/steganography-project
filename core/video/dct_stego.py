"""
Video DCT Steganography — Transform Domain.

Applies DCT-based embedding on selected video frames.
Uses motion-compensated frame selection and inter-frame consistency.
"""
...
class VideoDCT:
    """DCT-based video steganography with temporal awareness."""
    ...
    def __init__(
        ...
        self.image_dct = ImageDCT(alpha=alpha, seed=seed)
    ...
    def encode(
        ...
        for frame_idx in embed_indices:
            ...
            frames[frame_idx] = self.image_dct.encode(frames[frame_idx], chunk)
    ...
    def decode(self, video_path: str, max_frames: int = 300) -> bytes:
        """Extract data from DCT stego video."""
        frames, _ = extract_frames(video_path, max_frames=max_frames)
        embed_indices = list(range(0, len(frames), self.embed_every_n))

        first_chunk = self.image_dct.decode(frames[embed_indices[0]])
        ...
        for i in range(1, len(embed_indices)):
            if len(all_data) >= total_length:
                break
            chunk = self.image_dct.decode(frames[embed_indices[i]])
            all_data += chunk
