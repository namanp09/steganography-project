...
% ── Authors: all three on one centred block ─────────────────────────────────
\author{
...
steganography system that supports image, audio, and video carriers
...
encryption and Reed-Solomon error-correcting codes. Three independent
GAN models are trained for image, audio, and video under CPU-only
...
simulation evaluates all methods under 13 image, 10 audio, and 9 video
...
tradeoff is analysed across all ten method-modality combinations.
...
beyond images to audio and video carriers within a single unified
framework, and how such a system can be made deployable on
resource-constrained hardware (CPU-only inference, 512~MB RAM). We
build, train, and rigorously evaluate three GAN-based stego models
(image, audio, video) within a production-ready web application and
...
32 distinct attack configurations.
...
\begin{enumerate}
    ...
    single deployable system supporting image, audio, and video
    ...
    works.
    \item \textbf{Three GAN architectures optimised for CPU
    inference.} An image GAN (UNet encoder, PatchGAN discriminator,
    44~MB), an audio GAN operating on 128-bin mel-spectrograms
    (9~MB), and a video GAN with 3D temporal convolutions over a
    5-frame window (43~MB). All are trained from scratch on CPU
    hardware.
    ...
    Systematic evaluation across 32 attack configurations (13 image,
    10 audio, 9 video) using standardised PSNR and BER metrics—the
...
diffusion-model-based audio steganography \cite{b1}.
...
Cross-modal frameworks that uniformly support image, audio, and video
...
Comprehensive benchmarks across 30+ attack configurations using
standardised PSNR/BER metrics across all three media types are
...
\hline
L1 & Unified image+audio+video platform with three GAN models on shared API & III \\
L2 & CPU-only training and 512~MB-RAM CPU deployment for all three GANs & III \\
L3 & 32-attack cross-modal benchmark with PSNR/BER & IV \\
...
pipeline (Section~III-B), the three GAN architectures designed for
image, audio, and video carriers (Section~III-C), the adversarial
...
the Render cloud platform under a 512~MB-RAM free-tier. All three GAN
...
\subsection{Three GAN Architectures}

A central contribution of this work is the design of three
...
hardware. All three share the same conceptual structure: an encoder
...
\textbf{Audio GAN.} Audio waveforms are first transformed into
mel-spectrograms with $f_{\text{bins}}{=}128$ frequency bins (reduced
from the 513-bin full FFT specifically to make CPU training
feasible). The encoder and decoder operate as 1D convolutional
networks along the time axis with 32 base channels. The
spectrogram-domain output is inverse-transformed to a waveform via
Griffin–Lim. Checkpoint size is 9~MB.
...
\hline
\textbf{Parameter}   & \textbf{Image GAN} & \textbf{Audio GAN} & \textbf{Video GAN} \\
\hline
Input dims     & $64{\times}64{\times}3$ & 128 mel bins & $64{\times}64{\times}5$ \\
Message bits   & 128 & 128 & 128 \\
Base channels  & 32  & 32  & 16  \\
Conv type      & 2D UNet & 1D & 3D  \\
Temporal $T$   & --- & --- & 5 frames \\
Discriminator  & PatchGAN & PatchGAN & PatchGAN-3D \\
Checkpoint     & 44 MB & 9 MB & 43 MB \\
\hline
\end{tabular}
\label{tab_models}
\end{center}
...
All three models are trained jointly under a single compound loss:
\begin{equation}
...
are used for all three models, with the high $\lambda_1$ value
...
All three models are trained on CPU hardware. The image GAN uses
...
DWT—using their well-established constructions. LSB substitutes
message bits into the least
significant bits of pixel/sample values, giving high embedding PSNR
(70–99~dB) but no robustness. DCT embeds via quantisation index
modulation in mid-frequency $8{\times}8$ block coefficients to align
with the JPEG processing chain. DWT embeds in Haar wavelet detail
sub-bands (LH, HL) for images and Daubechies-4 detail coefficients
for audio. The classical baselines share the security pipeline of
Section~III-B; they are presented here as reference points for
quantitatively evaluating the GAN-based proposed methods.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{demo_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{chart_image.png}}
...
\subsection{Audio Steganography Results}

Ten attacks cover six categories: AWGN noise (SNR 30~dB and 20~dB),
low-pass filtering (4~kHz and 8~kHz), amplitude scaling ($\times$0.8
and $\times$1.2), resampling ($\div$2 then $\times$2), temporal shift
(100 samples), and echo addition (50~ms at 0.3$\times$ amplitude).
Audio GAN results are excluded due to a model checkpoint architecture
mismatch.

Table~\ref{tab_audio} and Fig.~\ref{fig_chart_audio} present results.

\begin{table}[htbp]
\caption{Audio Steganography BER Under Attacks (Lower is Better)}
\begin{center}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Attack}              & \textbf{LSB} & \textbf{DWT} \\
\hline
AWGN SNR = 30 dB             & 0.490 & \textbf{0.391} \\
AWGN SNR = 20 dB             & 0.465 & 0.486 \\
Low-Pass 4 kHz               & 0.518 & 0.518 \\
Low-Pass 8 kHz               & 0.479 & \textbf{0.000} \\
Volume $\times$0.8           & 0.490 & 0.525 \\
Volume $\times$1.2           & 0.514 & 0.496 \\
Resampling ($\div$2)         & 0.455 & \textbf{0.000} \\
Time Shift 100 samples       & 0.522 & 0.537 \\
Echo 50~ms (0.3$\times$)     & \textbf{0.455} & 0.500 \\
\hline
\textbf{Avg BER}             & 0.487 & \textbf{0.384} \\
\textbf{Embed PSNR}          & \textbf{86.74} & 49.92 \\
\hline
\end{tabular}
\label{tab_audio}
\end{center}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{chart_audio.png}}
\caption{BER under 9 audio attacks. DWT achieves avg BER~0.384
versus 0.487 for LSB (21.2\% improvement). DWT achieves BER $= 0.000$
under 8~kHz low-pass filtering and resampling — attacks that preserve
the frequency band in which DWT embeds. Both methods are destroyed by
time shifting and echo.}
\label{fig_chart_audio}
\end{figure}

DWT reduces average BER by 21.2\% relative to LSB. DWT achieves BER
$= 0.000$ under Low-Pass 8~kHz and Resampling because these operations
preserve signal content in the detail sub-band frequency range where
DWT embeds. Time shifting destroys both methods (BER $> 0.5$) by
displacing all samples from their expected index positions, causing
complete desynchronisation at the decoder.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{chart_video.png}}
...
average BER for each method across all three modalities.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{chart_summary_ber.png}}
...
(0.290); DWT leads for audio (0.384). Audio GAN and image/audio DCT
audio results are marked N/A where not applicable or not tested.}
...
\begin{table}[htbp]
\caption{Complete Cross-Modal Robustness Summary}
\begin{center}
...
\hline
Audio & LSB & 86.74 dB & 0.487 & Fragile \\
Audio & DWT & 49.92 dB & 0.384 & \textbf{Most Robust} \\
Audio & GAN & ---       & ---   & Not Tested \\
\hline
...
ten evaluated configurations.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{chart_psnr_tradeoff.png}}
\caption{Imperceptibility–robustness tradeoff for all ten
...
\textit{Cluster 1 — High PSNR, Fragile (top-right):} LSB across all
three modalities groups here with embed PSNR ranging from 70.27~dB
(image) to 98.79~dB (video) but average BER between 0.354 and 0.487,
...
at BER 0.180—is 51\% lower BER than LSB video (0.354) at a cost of
...
\begin{itemize}
    ...
    a balanced tradeoff is required across image, audio, or video.
...
integrating LSB, DCT, DWT, and GAN embedding across image, audio, and
...
evaluation framework covering 32 attack configurations across three
...
at Q=90 and Q=75. DWT is the strongest audio method (avg BER 0.384),
achieving perfect recovery under low-pass filtering and resampling.
LSB, despite providing the highest embedding quality (70--99~dB PSNR),
is classified Fragile or Moderate across all modalities. The
imperceptibility–robustness tradeoff confirms that no single method
simultaneously maximises both objectives: each configuration represents
a distinct operating point on the Pareto frontier.
...
still-image compression robustness. The audio GAN checkpoint could not
be fully evaluated due to an architecture mismatch. Planned extensions
include GPU retraining at 128$\times$128 resolution, JPEG noise layer
integration in the image GAN pipeline, synchronisation-robust audio
embedding using learned markers, and formal steganalysis evaluation
