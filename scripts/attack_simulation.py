...
For each medium (image, audio, video) and each method (LSB, DCT, DWT):
...
Results are printed as tables and saved to:
  outputs/attack_results_image.csv
  outputs/attack_results_audio.csv
  outputs/attack_results_video.csv
  outputs/attack_simulation_report.txt
"""
...
IMAGE_METHODS = [
    ("LSB",  ImageLSB(num_bits=2, seed=42)),
    ("DCT",  ImageDCT(alpha=10.0, seed=42)),
    ("DWT",  ImageDWT(wavelet="haar", level=2, alpha=5.0, seed=42)),
]

AUDIO_METHODS = [
    ("LSB",  AudioLSB(num_bits=2, seed=42)),
    ("DWT",  AudioDWT(wavelet="db4", level=4, alpha=0.02, seed=42)),
]

VIDEO_METHODS = [
    ("LSB",  VideoLSB(num_bits=2, embed_every_n=2, use_motion_comp=False, seed=42)),
    ("DCT",  VideoDCT(alpha=30.0, embed_every_n=2, use_motion_comp=False, seed=42)),
    ("DWT",  VideoDWT(wavelet="haar", level=2, alpha=10.0, embed_every_n=2, use_motion_comp=False, seed=42)),
]

HEADERS = ["Method", "Attack", "PSNR Stego→Attack (dB)", "PSNR Cover→Attack (dB)", "Recovered", "BER", "Time (ms)"]
...
def run_image_simulation():
    ...
    for method_name, method_obj in IMAGE_METHODS:
        print(f"\n  Method: {method_name}")
        try:
        ...
        for attack_name, attack_fn in IMAGE_ATTACKS:
            ...
            try:
                ...
                recovered = "YES" if decoded_msg == TEST_MESSAGE else "NO"
                ber = 0.0 if recovered == "YES" else compute_ber(payload, raw) if raw else 1.0
            except Exception as ex:
                psnr_s2a, psnr_c2a, recovered, ber = 0.0, 0.0, "ERROR", 1.0
            elapsed = round((time.time() - t0) * 1000, 1)
            row = [method_name, attack_name, psnr_s2a, psnr_c2a, recovered, round(ber, 4), elapsed]
            rows.append(row)
            all_rows.append(row)

        print_table(f"LSB/DCT/DWT — {method_name}  [embed PSNR: {embed_psnr} dB]", HEADERS, rows)
...
def run_audio_simulation():
    ...
    for method_name, method_obj in AUDIO_METHODS:
        print(f"\n  Method: {method_name}")
        try:
...
def run_video_simulation():
    ...
    for method_name, method_obj in VIDEO_METHODS:
        print(f"\n  Method: {method_name}")
        try:
        ...
        for attack_name, attack_fn in VIDEO_ATTACKS:
            ...
            try:
                ...
                psnr_s2a, psnr_c2a, recovered, ber = 0.0, 0.0, f"ERROR: {ex}", 1.0
            elapsed = round((time.time() - t0) * 1000, 1)
            row = [method_name, attack_name, psnr_s2a, psnr_c2a, recovered, round(ber, 4), elapsed]
            rows.append(row)
            all_rows.append(row)
...
def generate_charts(img_rows, audio_rows, video_rows):
    try:
        ...
        def plot_group(rows, title, out_file):
            ...
            # Robustness chart (recovery rate)
            ax2 = axes[1]
            for i, method in enumerate(methods):
                recovered = []
                for atk in attacks:
                    match = [r[4] for r in rows if r[0] == method and r[1] == atk]
                    recovered.append(100 if (match and match[0] == "YES") else 0)
                ax2.bar(x + i * w, recovered, w, label=method, color=colors[i % len(colors)], alpha=0.85)
            ax2.set_xticks(x + w)
            ax2.set_xticklabels(attacks, rotation=35, ha="right", fontsize=8)
            ax2.set_ylabel("Recovery Rate (%)")
            ax2.set_ylim(0, 110)
            ax2.set_title("Message Recovery After Attack")
            ax2.legend()
...
def write_report(img_rows, audio_rows, video_rows):
    ...
    def section(name, rows):
        ...
        for method in methods:
            ...
            total   = len(m_rows)
            success = sum(1 for r in m_rows if r[4] == "YES")
            avg_psnr = round(np.mean([r[2] for r in m_rows if isinstance(r[2], (int, float))]), 2)
            lines.append(f"  {method:6s} — Recovery: {success}/{total} attacks  |  Avg PSNR after attack: {avg_psnr} dB")

    section("IMAGE",  img_rows)
    section("AUDIO",  audio_rows)
    section("VIDEO",  video_rows)

    lines.append("\nNote: LSB is fragile to JPEG/compression attacks (0% recovery).")
    lines.append("      DCT/DWT survive JPEG compression partially.")
    lines.append("      GAN (trained with H.264 noise layer) is most robust.")
